...
app.post('/authenticate',async(request, reply) => {
    try{
        ...
        if(!user){
            return reply.status(409).send({menssage: 'Email não existe'})
        }
        const doesPasswordWacthes = await compare(password, user.password_hash)
        if(!doesPasswordWacthes){
            return reply.status(409).send({menssage: 'Credencias invalidas'})
        }
    }catch{}
})
